...
import { differenceInSeconds, subHours, addHours } from 'date-fns'
...
const validateFromTo = (newValue) => {
  ...
  if (fromDate && toDate) {
    ...
    if (diffInSeconds < 0) {
      ;[config.value.from, config.value.to] = [config.value.to, config.value.from]
      ui.notify.warning(t('조회기간 시작일시는 종료일시보다 작아야 합니다.'))
    } else if (Math.abs(diffInSeconds) > secondsInDay) {
      if (newValue === config.value.from) {
        config.value.from = subHours(toDate, 24)
      } else {
        config.value.to = addHours(fromDate, 24)
      }
      ui.notify.warning(t('조회기간은 24시간을 넘지 않아야 합니다.'))
    }
  }
}
