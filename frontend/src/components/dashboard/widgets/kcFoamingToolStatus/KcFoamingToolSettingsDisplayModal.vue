...
const processChunks = computed(() => {
  const chunks = []
  const processData = props.processValues.map((process) => ({
  ...
  for (let i = 0; i < processData.length; i += props.columnsPerTable) {
    chunks.push(processData.slice(i, i + props.columnsPerTable))
  }
