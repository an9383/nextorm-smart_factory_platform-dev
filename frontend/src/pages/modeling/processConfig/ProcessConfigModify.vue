  ...
  <q-dialog v-model="IS_VISIBLE" persistent>
    <q-card class="dialog-container">
      <q-card-section class="bg-secondary text-white">
        <div class="text-h6">{{ $t('Process') }} {{ $t(dialogModeName) }}</div>
      </q-card-section>
      <q-card-section>
        <q-form ref="form">
...
import ProcessConfigService from 'src/services/modeling/ProcessConfigService'
...
const handleOkClick = async () => {
  ...
  try {
    ...
    dialogMode === CREATE_MODE
      ? await ProcessConfigService.createProcessConfig(body)
      : await ProcessConfigService.modifyProcessConfig(configId.value, body)
...
onBeforeMount(async () => {
  if (configId.value) {
    const configs = await ProcessConfigService.getProcessConfigs()
