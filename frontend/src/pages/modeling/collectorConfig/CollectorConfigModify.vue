  ...
  <q-dialog v-model="IS_VISIBLE" persistent>
    <q-card class="dialog-container">
      <q-card-section class="bg-secondary text-white">
        <div class="text-h6">{{ $t('Collector') }} {{ $t(dialogModeName) }}</div>
      </q-card-section>
      <q-card-section>
        <q-form ref="form">
...
const handleOkClick = async () => {
  ...
  try {
    if (dialogMode === CREATE_MODE) {
      await CollectorConfigService.createCollectorConfig(body)
    } else {
      await CollectorConfigService.modifyCollectorConfig(configId.value, body)
    }
...
onBeforeMount(async () => {
  if (configId.value) {
    const configs = await CollectorConfigService.getCollectorConfigs()
