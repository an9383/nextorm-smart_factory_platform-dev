...
@SuperBuilder
@Getter
public class CollectorConfig extends BaseEntity {
	@Column(name = "name", unique = true)
	private String name;

	@Builder.Default
	@OneToMany(fetch = FetchType.LAZY, mappedBy = "collectorConfig", cascade = {CascadeType.ALL}, orphanRemoval = true)
	private List<CollectorConfigToolMapping> toolMappings = new ArrayList<>();
	...
	public List<Tool> getTools() {
		return toolMappings.stream()
						   .map(CollectorConfigToolMapping::getTool)
						   .toList();
	}

	public void addTools(List<Tool> tools) {
		toolMappings.addAll(tools.stream()
								 .map(tool -> CollectorConfigToolMapping.create(this, tool))
								 .toList());
	}

	public static CollectorConfig create(CollectorConfig createData) {
		return CollectorConfig.builder()
							  .name(createData.getName())
							  .isUseFailover(createData.isUseFailover())
	...
	public void modify(
		...
		if (Objects.nonNull(modifyData)) {
			...
			this.isUseFailover = modifyData.isUseFailover();
